...
with st.sidebar:
    ...
    if tipo_selecionado == "📊 Dados Gerais":
        ...
        anos_validos = [
            int(v)
            for v in pd.concat(
                [df_tcc['ano'], df_art['ano']], ignore_index=True
        ...
        if tipo_selecionado == "📚 TCCs":
            cursos = st.multiselect("Cursos", sorted(df['curso_unificado'].dropna().unique()))
            tipos = sorted(df['tipo'].dropna().unique())

# ── FILTRAR ───────────────────────────────────────────────────────────────────
if tipo_selecionado == "📊 Dados Gerais":
    df_tcc = filtrar_dados(df_tcc, inst, anos if anos else (0, 9999), topicos)
    df_art = filtrar_dados(df_art, inst, anos if anos else (0, 9999), topicos)
    df_proj = filtrar_dados(df_proj, inst, None, topicos)
    df_filtrado = df_tcc
else:
